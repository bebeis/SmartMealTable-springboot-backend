...
CREATE TABLE IF NOT EXISTS member_profile (
    ...
    CONSTRAINT fk_memberprofile_user FOREIGN KEY (user_id)
    REFERENCES member_auth(user_id)
...
CREATE TABLE IF NOT EXISTS social_login (
    ...
    CONSTRAINT fk_sociallogin_user FOREIGN KEY (user_id)
    REFERENCES member_auth(user_id)
                                                            ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS member_address (
    ...
    CONSTRAINT fk_memberaddress_user FOREIGN KEY (user_id)
    REFERENCES member_auth(user_id)
                                                            ON DELETE CASCADE ON UPDATE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS monthly_budget (
    ...
    UNIQUE KEY uq_user_yearmonth (user_id, year_month),
    CONSTRAINT fk_monthlybudget_user FOREIGN KEY (user_id)
    REFERENCES member_auth(user_id)
                                                            ON DELETE CASCADE ON UPDATE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS daily_budget (
    ...
    UNIQUE KEY uq_user_date (user_id, budget_date),
    CONSTRAINT fk_dailybudget_user FOREIGN KEY (user_id)
    REFERENCES member_auth(user_id)
                                                            ON DELETE CASCADE ON UPDATE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS food_store (
    ...
    store_type ENUM('RESTAURANT', 'SCHOOL_CAFETERIA', 'CONVENIENCE_STORE') NOT NULL, -- 음식점 타입
...
CREATE TABLE IF NOT EXISTS favorite_food_store (
    ...
    REFERENCES food_store(food_store_id)
    ON DELETE CASCADE ON UPDATE CASCADE,
    UNIQUE KEY uq_favorite (user_id, food_store_id)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS expenditure (
    ...
    CONSTRAINT fk_expenditure_user FOREIGN KEY (user_id)
    REFERENCES member_auth(user_id)
                                                            ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS expenditure_food (
                                                ...
                                                unit_price DECIMAL(10,2) NOT NULL,  -- 주문 당시 단위 가격 (Food 테이블의 가격과 동일할 수도 있으나, 주문 시점의 가격을 기록)
    total_price DECIMAL(10,2) NOT NULL, -- quantity * unit_price 계산 값 (혹은 주문 당시 총액)
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
-- 6.1. 사용자 음식 취향 선호 테이블 (UserFoodPreference)
CREATE TABLE IF NOT EXISTS user_food_preference (
    ...
    REFERENCES member_auth(user_id)
                                                            ON DELETE CASCADE ON UPDATE CASCADE,
    UNIQUE KEY uq_user_food (user_id, food_keyword)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;